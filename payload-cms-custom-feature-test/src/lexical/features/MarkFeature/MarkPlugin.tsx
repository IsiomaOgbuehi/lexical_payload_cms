...
import { $getSelection, $isRangeSelection, $isTextNode, COMMAND_PRIORITY_EDITOR, createCommand, LexicalCommand, TextNode } from 'lexical'
    ...
    return editor.registerCommand(
      ...
      () => {
        ...
        if (hasHighlight) {
          ...
          // Add highlight: convert text nodes to highlighted nodes
          nodes.forEach(node => {
            if ($isTextNode(node)) {
              const markNode = $createMarkNode(node.getTextContent())
              markNode.setFormat(node.getFormat())
              markNode.setStyle(node.getStyle())
